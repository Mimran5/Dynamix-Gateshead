...
export const forceUpdateClasses = async () => {
  console.log('Force updating classes with correct data...');
  const classesRef = collection(db, 'classes');
  
  try {
    // First, delete all existing classes
    const existingClasses = await getDocs(classesRef);
    for (const doc of existingClasses.docs) {
      await deleteDoc(doc.ref);
      console.log(`Deleted existing class: ${doc.id}`);
    }
    
    // Then create all classes with correct data
    ...
    console.log('All classes force updated successfully!');
  } catch (error) {
